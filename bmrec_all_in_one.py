...
# pyinstaller -F -w  --add-data "frontend_gui.htm;." bmrec_all_in_one.py
# 迁移
...
CONTROL_FILE = './bmrec_control.tmp'
LOG_PATH = './logs'
...
NO_ALARM = r'<img src=\"\/public\/images\/redAlm.png\"><label class=\"redAlm\">0<\/label><img src=\"\/public\/images\/ylwAlm.png\"><label class=\"ylwAlm\">0<\/label>'
...
class AppState:
    def __init__(self):
        ...
        self.alarm_log = []  # 设备报警记录
        self.alarm_status = {}  # 设备报警状态
        self.group_alarm_lines = []  # 报警组合发送暂存
    ...
    def setup_logger():
        ...
        handler = logging.handlers.TimedRotatingFileHandler(os.path.join(
            LOG_PATH, 'monitor.log'), encoding='utf-8', when='W0', interval=1, backupCount=52)
...
class MonitorSystem:
    ...
    def load_config(self):
        ...
        # 服务器设置
        self.serv_port = app_state.config.get(
            "Config", "serv_port", fallback="5000"),
        self.alert_group_send = app_state.config.get(
            "Config", "alert_group_send", fallback="NO"),
        self.servers = app_state.config.get(
            "Config", "servers", fallback="127.0.0.1").split(",")
        self.server_names = app_state.config.get(
            "Config", "names", fallback="INVT集成机柜").split(",")
        self.users = app_state.config.get(
            "Config", "users", fallback="admin").split(",")
        self.passwords = app_state.config.get(
            "Config", "passwords", fallback="123456").split(",")
        ...
        app_state.current_settings = {
            ...
            "alert_group_send": self.alert_group_send,
    ...
    def save_config(self, new_settings):
        ...
        app_state.config["Config"] = {
            ...
            "alert_group_send": new_settings["alert_group_send"],
    ...
    def save_config_one(self, sectionname, configname, new_settings):
        app_state.config.set(sectionname, configname, new_settings)
        with open(CONFIG_PATH, "w", encoding="utf-8") as configfile:
            app_state.config.write(configfile)
    ...
    def get_server_data(self, server, server_name_last="", user_last="", password_last=""):
        if "YES" in str(self.alert_group_send):
            send_in_group = True
        else:
            send_in_group = False
        # print("send_in_group",send_in_group)
        result = ""
        try:
        ...
        try:
            ...
            with requests.Session() as r_session:
                ...
                r = r_session.post(
                    login_url, data={"usern": user, "psw": password}, timeout=10, verify=False)
                ...
                if r.status_code == 200:
                    ...
                    r = r_session.post(
                        alarm_url, data={"alm_sts": "1"}, verify=False).text
                    ...
                    if NO_ALARM not in r:
                        ...
                        alm_log_new = server_name+str(hashlib.md5(content.encode("utf-8")).hexdigest())+str(
                        ...
                        if alm_log_new in app_state.alarm_log:
                            ...
                            if send_in_group:
                                app_state.group_alarm_lines.append(content)
                        ...
                        try:
                            if (app_state.alarm_status[server] == 1):
                                ...
                                m_subject = f'动环报警-{server_name}'
                                m_content = '报警解除'
                                alm_log_new = server_name+':'+m_content + \
                                    str(time.strftime(
                                ...
                                if send_in_group:
                                    app_state.group_alarm_lines.append(
                                        m_content)
                    ...
                    app_state.alarm_status[server] = 1
                    alm_log_new = server_name+'-设备连接异常-' + \
                        str(time.strftime("%Y-%m-%d %H", time.localtime()))
                    if alm_log_new in app_state.alarm_log:
                        pass
                    else:
                        app_state.logger.info(alm_log_new)
                        ...
                        if send_in_group:
                            app_state.group_alarm_lines.append(
                                f"{server_name}设备连接异常，请检查网络连接或设备状态。{str(time.strftime('%Y-%m-%d %H', time.localtime()))}")
            ...
            if alm_log_new in app_state.alarm_log:
                ...
                if send_in_group:
                    app_state.group_alarm_lines.append(m_content)
                    pass
    ...
    def _parse_temp_humi(self, data):
        try:
            ...
            state_th = str(dataj["hotData"]) if str(
                dataj["hotData"]) != "[]" else str(dataj["coldData"])
    ...
    def send_email(self, subject, content, to_mail, smtp_host, smtp_port, mail_user, mail_pass, sender_email, smtp_type):
        try:
            ...
            message["To"] = ",".join(to_mail) if isinstance(
                to_mail, list) else to_mail
...
class SettingsWindow:
    ...
    def create_widgets(self):
        ...
        ttk.Label(server_frame, text="WEB服务端口:").grid(
            row=0, column=0, padx=5, pady=5, sticky='e')
        self.servport_entry = ttk.Entry(server_frame, width=60)
        self.servport_entry.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(server_frame, text="报警组发送:").grid(
            row=1, column=0, padx=5, pady=5, sticky='e')
        self.alert_group_send_entry = ttk.Combobox(
            server_frame, values=["NO", "YES"], width=57)
        self.alert_group_send_entry.grid(row=1, column=1, padx=5, pady=5)
        self.alert_group_send_entry.set("NO")

        ttk.Label(server_frame, text="监控地址(多个用逗号分隔):").grid(
            row=2, column=0, padx=5, pady=5, sticky='e')
        self.servers_entry = ttk.Entry(server_frame, width=60)
        self.servers_entry.grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(server_frame, text="监控名称(多个用逗号分隔):").grid(
            row=3, column=0, padx=5, pady=5, sticky='e')
        self.names_entry = ttk.Entry(server_frame, width=60)
        self.names_entry.grid(row=3, column=1, padx=5, pady=5)

        ttk.Label(server_frame, text="监控用户名(多个用逗号分隔):").grid(
            row=4, column=0, padx=5, pady=5, sticky='e')
        self.users_entry = ttk.Entry(server_frame, width=60)
        self.users_entry.grid(row=4, column=1, padx=5, pady=5)

        ttk.Label(server_frame, text="监控密码(多个用逗号分隔):").grid(
            row=5, column=0, padx=5, pady=5, sticky='e')
        self.passwords_entry = ttk.Entry(server_frame, width=60)  # , show="*"
        self.passwords_entry.grid(row=5, column=1, padx=5, pady=5)
        ...
        ttk.Label(email_frame, text="收件人邮箱(多个用逗号分隔):").grid(
            row=5, column=0, padx=5, pady=5, sticky='e')
        self.email_receivers_entry = ttk.Entry(email_frame, width=60)
        self.email_receivers_entry.grid(row=5, column=1, padx=5, pady=5)

        ttk.Label(email_frame, text="SMTP类型:").grid(
            row=6, column=0, padx=5, pady=5, sticky='e')
        self.smtptype_combobox = ttk.Combobox(
            email_frame, values=["SSL", "TLS", "None"], width=57)
        ...
        ttk.Button(button_frame, text="保存", command=self.save_settings).pack(
            side='right', padx=5)
        ttk.Button(button_frame, text="取消", command=self.on_close).pack(
            side='right', padx=5)

    def load_settings(self):
        self.servport_entry.insert(0, app_state.current_settings["serv_port"])
        self.alert_group_send_entry.set(
            app_state.current_settings["alert_group_send"])
        ...
        self.sender_email_entry.insert(
            0, app_state.current_settings["sender_email"])
        self.email_receivers_entry.insert(
            0, app_state.current_settings["email_receivers"])
        self.smtptype_combobox.set(app_state.current_settings["smtptype"])

    def save_settings(self):
        new_settings = {
            "serv_port": self.servport_entry.get(),
            "alert_group_send": self.alert_group_send_entry.get(),
...
def create_flask_app():
    ...
    def serve_frontend():
        ...
        if os.path.exists(template_path):
            return flask.send_from_directory(os.path.dirname(template_path),
    ...
    while port_available == False:
        ...
        if port_available == False:
            if app_port == app_port_init and app_port != SERV_PORT:
                ...
                app_port += 1
    app_state.monitor_system.save_config_one(
        "Config", "serv_port", str(app_port))
    app.run(host='0.0.0.0', port=app_port, debug=True, use_reloader=False)


def monitor_worker():
    while True:
        # print("监控工作线程运行中..."+app_state.config.get("Config", "alert_group_send", fallback="NO"))
        if app_state.config.get(
                "Config", "alert_group_send", fallback="NO") == "YES":
            send_in_group = True
            email_settings = {
                "smtp_host": app_state.config.get("Email", "smtp_host", fallback="smtp.qq.com"),
                "smtp_port": app_state.config.get("Email", "smtp_port", fallback="465"),
        ...
        try:
            result_new = '{"num":"' + \
                str(len(app_state.monitor_system.servers)) + '","servers":['
            ...
            if send_in_group:
                # print("报警组发送", app_state.group_alarm_lines)
                if app_state.group_alarm_lines != []:
                    alarm_text = ""
                    for group_alarm_line in app_state.group_alarm_lines:
                        alarm_text += group_alarm_line+'\n'
                    m_subject = f'动环报警-报警组'
                    m_content = alarm_text
                    alm_log_new = server_name_last+str(hashlib.md5(alarm_text.encode("utf-8")).hexdigest())+str(
                        time.strftime("%Y-%m-%d %H", time.localtime()))
                    app_state.logger.info(alm_log_new)
                    ...
                    if alm_log_new in app_state.alarm_log:
                        ...
                        send_email(
                            m_subject, m_content,
                            email_settings["email_receivers"],
                            email_settings["smtp_host"],
                            email_settings["smtp_port"],
                            email_settings["mail_user"],
                            email_settings["mail_pass"],
                            email_settings["sender_email"],
                            email_settings["smtptype"]
                        )
                        app_state.alarm_log.append(alm_log_new)
                    app_state.group_alarm_lines = []
        except Exception as e:
...
def send_email(subject, content, to_mail, smtp_host, smtp_port, mail_user, mail_pass, sender_email, smtp_type):
    try:
        message = MIMEMultipart()
        message["From"] = sender_email
        message["To"] = ",".join(to_mail) if isinstance(
...
def on_show_web(icon, item):
    webbrowser.open("http://localhost:" +
                    app_state.config.get("Config", "serv_port", fallback="5000"))
...
def main():
    ...
    def start_browser():
        while 1 == 1:
            try:
                r = requests.get("http://localhost:"+app_state.config.get("Config",
                                 "serv_port", fallback="5000")+"/dev_list?callback=1233")
                if r.status_code == 200:
                    break
                else:
                    time.sleep(1)
...
def cli_show_status():
    print("监控系统状态:")
    try:
        r = requests.get("http://localhost:"+str(SERV_PORT) +
                         "/dev_list?callback=show", timeout=5)
        if r.status_code == 200:
            print("监控系统正在运行")
        else:
            print("监控系统未运行")
        ...
        if os.path.exists(CONTROL_FILE):
            with open(CONTROL_FILE, 'r', encoding='utf-8') as f:
                ...
                if "stop" in flag:
                    print("监控系统已停止")
                    os._exit(0)
                elif "restart" in flag:
                    print("监控系统已重启")
                    app_state.monitor_thread.stop()
                    app_state.flask_thread.stop()
                    time.sleep(3)
...
def cli_run_server():
    ...
    time.sleep(3)
    # 检查控制文件
    app_state.check_thread = threading.Thread(
        target=cli_check_control_file, daemon=True)
    app_state.check_thread.start()
...
if __name__ == "__main__":
    ...
    try:
        if args.action == "start":
            app_state.logger.info("启动监控系统")
            cli_run_server()
        elif args.action == "stop":
            app_state.logger.info("停止监控系统")
            cli_stop_server()
            os._exit(0)
        elif args.action == "restart":
            app_state.logger.info("重启监控系统")
            cli_stop_server()
            time.sleep(5)
            if check_executable():
                print('exe:'+get_executable_path())
                # 获取操作系统名称

                if system == 'windows':
                    # print("当前操作系统是 Windows")
                    os.system('start /B ./'+get_executable_path() +
                              ' -a start > NUL 2>&1')
                else:
                    # print("当前操作系统是 Linux")
                    os.system('nohup ./' + get_executable_path() +
                              ' -a start > /dev/null 2>&1 &')
            else:
                print('py:'+get_executable_path())
                if system == 'windows':
                    # print("当前操作系统是 Windows")
                    os.system('start /B python ./' +
                              get_executable_path()+' -a start > NUL 2>&1')
                else:
                    # print("当前操作系统是 Linux")
                    os.system('nohup python ./' + get_executable_path() +
                              ' -a start > /dev/null 2>&1 &')
            ...
            app_can_run = False
            app_state.logger.info("查看监控系统状态")
            cli_show_status()
            os._exit(0)
        elif args.action == "":
            app_can_run = True
        else:
            print("BMRec 命令行参数\n bmrec_all_in_one 不带参数启动GUI\n bmrec_all_in_one -a ACTION\n\nACTION:\n  start: 启动监控系统\n  stop: 停止监控系统\n  restart: 重启监控系统\n  status: 查看监控系统状态")
    except Exception as e:
        app_can_run = True
